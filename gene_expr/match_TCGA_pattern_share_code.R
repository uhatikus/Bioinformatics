...
for (cutoff_value in cutoff_values) {
  ...
  p_error <- sum(errors_matched) / length(useful_exp_data$hgnc_symbol) / nrow(tcga_data_matched)
  ...
  p_error <- sum(errors_unmatched) / length(useful_exp_data$hgnc_symbol) / nrow(tcga_data_unmatched)
...
p <- ggplot() +
  ...
  geom_point(data = data.frame(x = cutoff_values, y = p_errors_unmatched), aes(x = cutoff_values, y = p_errors_unmatched), color = "green") +
  ...
  labs(title = "Scatter Plot with Two Sets of Points", x = "X", y = "Y") +
  theme_minimal()
IRdisplay::display(p)
