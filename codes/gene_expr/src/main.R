...
tcga_data_init <- read.table("data/tcga_lgg_data_old.txt", header = TRUE, sep = "\t")
cgga_data_init <- read.table("data/cgga_data_mRNAseq.txt", header = TRUE, sep = "\t")
# cgga_data_init <- read.table("data/cgga_data_mRNA_array.txt", header = TRUE, sep = "\t")
...
for (sheet in all_sheets) {
  ...
  for (cutoff_value in cutoff_values) {
    ...
    h_ration <- summary(res.cox)$coefficients["group",2]
  ...
  ggsave(paste0("plots/results/", sheet, "_cgga_mRNAseq.png"), combined_plot, width = 18, height = 26, dpi = 300)
  
  # break
}
# TODO:
# Survival analysis by each gene in Progress vs None: table hazard ratio
# 1. hist type (?)
...
# X: expression mRNAseq. Y: expression mRNA array
