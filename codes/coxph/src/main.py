...
def test_one_gene(gene):
    coxph = CoxPH(
        clinical_file_path=f"{root_folder}data/lgg/lgg_clinical_with_rows.csv",
...
def test_unique_genes_from_dataset():
    ...
    unique_genes = set(coxph.mutation_df[~mask]["Hugo_Symbol"].unique())
...
def test_sum_mutations():
    ...
    unique_genes_from_datatset = set(coxph.mutation_df["Hugo_Symbol"].unique())
...
def skcm_analysis():
    coxph = CoxPH(
        clinical_file_path=f"{root_folder}data/skcm/skcm_clinical_with_rows.csv",
        mutation_file_path=f"{root_folder}data/skcm/skcm_mutation_with_rows.csv",
        results_path=f"{root_folder}results/skcm_results/",
        skip_mutations=True,
    )
    ...
    coxph.analyze_by_two_groups(
        defined_group=people, covariats=["gender", "years_to_birth"]
    )  # , 'stage', 'grade'


def lgg_gene_expression():
    coxph = CoxPH(
        clinical_file_path=f"{root_folder}data/lgg_2/tcga_lgg_data.csv",
        mutation_file_path=f"...",
        results_path=f"{root_folder}results/lgg_2_results_res_MT_Progress/",
        skip_mutations=True,
    )
    ...
    for cutoff_n in [0.6, 0.65, 0.7, 0.75, 0.8, 0.85]:
        ...
        print(cutoff)
        with open(
            f"{root_folder}given_files/lgg_matched_cutoff_res_MT_Progress {cutoff} .txt",
            "r",
        ) as file:
            people_matched = [gene.strip() for gene in file.readlines()]

        with open(
            f"{root_folder}given_files/lgg_unmatched_cutoff_res_MT_Progress {cutoff} .txt",
            "r",
        ) as file:
        ...
        coxph.analyze_by_two_defined_groups(
            group1=people_matched,
            group2=people_unmatched,
            covariats=["gender"],
            cutoff=cutoff,
        )  # , 'stage', 'grade'
